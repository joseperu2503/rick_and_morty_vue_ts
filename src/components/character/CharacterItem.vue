<template>
  <div @click="goCharacter()" class="relative cursor-pointer hover:scale-110 transition-all w-full aspect-square"
    ...
    <img :src="character.image" class="rounded-2xl w-full" @load="onImageLoad" @error="onImageError"
      v-show="imageLoaded" />
    <div class="absolute bg-gradient-to-t from-black bottom-0 h-1/5 w-full px-4 flex items-end pb-4 rounded-b-2xl">
    </div>
    <div class="absolute bg-gradient-to-t from-black bottom-0 opacity-50 h-2/5 w-full px-4 flex items-end pb-4 rounded-b-2xl">
    </div>
    <div class="absolute bottom-0 h-2/5 w-full px-4 flex items-end pb-4 rounded-b-2xl">
